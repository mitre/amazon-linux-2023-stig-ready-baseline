control 'AMZL-23-230229' do
  title 'Amazon Linux 2023, for PKI-based authentication, must validate certificates by
...
requirement.'
  desc 'check', 'Verify Amazon Linux 2023 for PKI-based authentication has valid certificates by
constructing a certification path (which includes status information) to an
accepted trust anchor.
...
installed in the /etc/sssd/pki/sssd_auth_ca_db.pem location, this is a finding.'
  desc 'fix', 'Configure Amazon Linux 2023, for PKI-based authentication, to validate certificates by
constructing a certification path (which includes status information) to an
accepted trust anchor.
...
bundle from cyber.mil and copy the DoD_PKE_CA_chain.pem into the following file:
  ...
  tag gid: 'AMZL-23-230229'
  tag rid: 'AMZL-23-230229r627750_rule'
  tag stig_id: 'AMZL-23-010090'
