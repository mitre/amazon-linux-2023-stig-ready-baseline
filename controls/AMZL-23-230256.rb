control 'AMZL-23-230256' do
  title 'The Amazon Linux 2023 operating system must implement DoD-approved TLS encryption
...
maintaining the confidentiality of the secret key used to generate the hash.
 ...
 SSLv2 is not available in the GnuTLS library.  The Amazon Linux 2023 system-wide crypto
...
disable unapproved SSL/TLS versions, this is a finding.'
  desc 'fix', 'Configure the Amazon Linux 2023 GnuTLS library to use only DoD-approved encryption by
adding the following line to "/etc/crypto-policies/back-ends/gnutls.config":
  ...
  tag gid: 'AMZL-23-230256'
  tag rid: 'AMZL-23-230256r627750_rule'
  tag stig_id: 'AMZL-23-010295'
  tag fix_id: 'F-32900r567515_fix'
  tag cci: ['CCI-001453']
  tag nist: ['AC-17 (2)']

  describe file('/etc/crypto-policies/back-ends/gnutls.config') do
    its('content') { should match(/enabled-version\s*=\s*TLS1\.3/i) }
    its('content') { should match(/enabled-version\s*=\s*TLS1\.2/i) }
    its('content') { should match(/enabled-version\s*=\s*DTLS1\.2/i) }
  end
end
