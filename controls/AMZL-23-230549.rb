control 'AMZL-23-230549' do
  title 'Amazon Linux 2023 must use reverse path filtering on all IPv4 interfaces.'
...
are not routable.  There is not an equivalent filter for IPv6 traffic.'
  desc 'check', 'Verify Amazon Linux 2023 uses reverse path filtering on all IPv4 interfaces with the
following commands:
...
returned, this is a finding.'
  desc 'fix', 'Configure Amazon Linux 2023 to use reverse path filtering on all IPv4 interfaces by
...
effect. To reload the contents of the files, run the following command:
  ...
  tag gid: 'AMZL-23-230549'
  tag rid: 'AMZL-23-230549r627750_rule'
  tag stig_id: 'AMZL-23-040285'
