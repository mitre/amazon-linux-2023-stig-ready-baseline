control 'AMZL-23-230252' do
  title 'The Amazon Linux 2023 operating system must implement DoD-approved encryption to
...
example above, they are missing, or commented out, this is a finding.)
  desc 'fix', 'Configure the Amazon Linux 2023 SSH server to use only ciphers employing FIPS
...
line:
  ...
  tag gid: 'AMZL-23-230252'
  tag rid: 'AMZL-23-230252r743940_rule'
  tag stig_id: 'AMZL-23-010291'
  ...
  if virtualization.system.eql?('docker') && !file('/etc/sysconfig/sshd').exist?
    ...
    describe sshd_config('/etc/crypto-policies/back-ends/opensshserver.config') do
      its('Ciphers') { should include('aes256-gcm@openssh.com') }
      its('Ciphers') { should include('aes256-ctr') }
      its('Ciphers') { should include('aes128-gcm@openssh.com') }
