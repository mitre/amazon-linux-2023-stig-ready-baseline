control 'AMZL-23-230392' do
  title 'The Amazon Linux 2023 audit system must take appropriate action when the audit
storage volume is full.'
  desc 'It is critical that when Amazon Linux 2023 is at risk of failing to process audit
...
Amazon Linux 2023 must continue generating audit records if possible (automatically
...
communication with this server is lost or the server fails, Amazon Linux 2023 must queue
...
audit data with the collection server.'
  desc 'check', 'Verify Amazon Linux 2023 takes the appropriate action when the audit storage volume is
full.

    Check that Amazon Linux 2023 takes the appropriate action when the audit storage
...
a finding.'
  desc 'fix', 'Configure Amazon Linux 2023 to shut down by default upon audit failure (unless
...
by setting the "disk_full_action" to "SYSLOG".'
  ...
  tag gid: 'AMZL-23-230392'
  tag rid: 'AMZL-23-230392r627750_rule'
  tag stig_id: 'AMZL-23-030060'
