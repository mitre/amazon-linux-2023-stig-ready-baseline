control 'AMZL-23-230530' do
  title 'The x86 Ctrl-Alt-Delete key sequence in Amazon Linux 2023 must be disabled if a
...
reduced because the user will be prompted before any action is taken.'
  desc 'check', %q(Verify Amazon Linux 2023 is not configured to reboot the system when Ctrl-Alt-Delete
is pressed when using a graphical user interface with the following command:

    This requirement assumes the use of the Amazon Linux 2023 default graphical user
...
user interface:
  ...
  tag gid: 'AMZL-23-230530'
  tag rid: 'AMZL-23-230530r646883_rule'
  tag stig_id: 'AMZL-23-040171'
